...
@section('content')
<?php
    $profesores=DB::select('SELECT u.id as idUser, u.name as name, u.location as location, u.pricing as pricing, u.rating as rating FROM users as u JOIN usersubject us ON u.id=us.id_user JOIN subject s ON us.id_subject=s.id WHERE s.id='.$_GET["asignatura"]);
                    ...
                    <div class="col-lg-12">
                        <div class="search-result">
                            <div class="result-body">
                                <form id="search-form" method="get" action='perfilEditar'>
                                    ...
                                    foreach ($profesores as $profesor){
                                        ...
                                        echo '<td >
                                                <form id="search-form" method="get" action=\'perfilEditar\'>
                                                    ...
                                                    <input type=\'hidden\' value="'.$idEnviar.'" id="user"name=user>
                                                    <button type="submit " class="btn btn-link">  Ir </button></td>
                                                </form></td>';
                                        echo '<td>'.$profesor->location.'</td>';
                                        echo '<td>'.$profesor->pricing.'</td>';
                                        echo '<td><div class="row"><span>'.$profesor->rating.'</span>';
